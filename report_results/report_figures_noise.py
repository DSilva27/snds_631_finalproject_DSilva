...
def run_noise_test(vol_fname, n_iter, config, signal_noise_ratios, param_setups):
    ...
    results = {
        "true_mtx": np.zeros((len(signal_noise_ratios), len(param_setups), n_iter, 3, 3)),
        "optim_mtx_wemd": np.zeros((len(signal_noise_ratios), len(param_setups), n_iter, 3, 3)),
        "refin_mtx_wemd": np.zeros((len(signal_noise_ratios), len(param_setups), n_iter, 3, 3)),
        "optim_mtx_l2": np.zeros((len(signal_noise_ratios), len(param_setups), n_iter, 3, 3)),
        "refin_mtx_l2": np.zeros((len(signal_noise_ratios), len(param_setups), n_iter, 3, 3)),
    ...
    for i, snr in enumerate(signal_noise_ratios):
        ...
        for j, param in enumerate(param_setups):
            ...
            for k in range(n_iter):
                ...
                # add noise
                vol = vol + np.random.normal(0, noise_std, vol._data[0].shape).astype(np.float32)
                vol_ref_noisy = vol_ref + np.random.normal(0, noise_std, vol_ref._data[0].shape).astype(np.float32)
                ...
                error_wemd_opt = calc_error(results["optim_mtx_wemd"][i, j, k], results["true_mtx"][i, j, k])
                error_wemd_ref = calc_error(results["refin_mtx_wemd"][i, j, k], results["true_mtx"][i, j, k])
                error_l2_opt = calc_error(results["optim_mtx_l2"][i, j, k], results["true_mtx"][i, j, k])
                error_l2_ref = calc_error(results["refin_mtx_l2"][i, j, k], results["true_mtx"][i, j, k])

                logging.info(f"Results for volume {vol_fname}, snr = {snr}, parameters (downsample_res, max_iter) = {param}, iteration {k}:")
                logging.info(f"True rotation matrix: {results['true_mtx'][i, j, k]}")
                logging.info(f"Optim rotation matrix for wemd: {results['optim_mtx_wemd'][i, j, k]}")
                logging.info(f"Refined rotation matrix for wemd: {results['refin_mtx_wemd'][i, j, k]}")
                logging.info(f"Optim rotation matrix for l2: {results['optim_mtx_l2'][i, j, k]}")
                logging.info(f"Refined rotation matrix for l2: {results['refin_mtx_l2'][i, j, k]}")
